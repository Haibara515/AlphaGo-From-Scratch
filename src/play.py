...
class GoPygame:
    ...
    def __init__(self, size: int = 19,
        ...
        self.ai_vs_ai_mode = False  # 显示状态：AI vs AI（双模型）时用于界面文案
        self.mode_title = None      # 显示状态：自定义模式标题（功能 7 使用；None 时不影响原有文案）
    ...
    def _init_ai(self):
        ...
        try:
            ...
            self.ai = PolicyNetworkAlphaGoLee(input_channels=2, num_actions=self.size * self.size + 1)
    ...
    def render(self):
        ...
        if self.mode_title:
            ai_status = self.mode_title
        elif self.ai_vs_ai_mode:
            ai_status = "🤖 AI vs AI（双模型）"
        else:
...
def board_to_model_input(board, current_player):
    """
    将棋盘状态转换为模型输入格式（与 Strategy_Network_2.py 的数据约定一致）。

    Args:
        board: size×size 棋盘（0=空, 1=黑, 2=白），可以是列表或 numpy 数组
        current_player: 当前行棋方（1=黑, 2=白）

    Returns:
        state: (2, size, size) float32 数组
            通道 0 = 当前行棋方的棋子
            通道 1 = 对方棋子
    """
    import numpy as np
...
def _load_ai_model(model_path, board_size=19):
    """
    加载策略网络模型（PolicyNetworkAlphaGoLee），失败时打印错误并返回 None。

    支持两种检查点格式：
    - 训练脚本保存的完整字典（含 'model_state_dict'）
    - 裸 state_dict
    """
    if torch is None:
        print("⚠️ 未安装 torch，无法加载 AI 模型（将使用占位逻辑）")
        return None

    try:
        # 与现有 AI 逻辑一致：从 Strategy_Network_2.py 导入模型类
        from src.policy_network import PolicyNetworkAlphaGoLee

        model = PolicyNetworkAlphaGoLee(
            input_channels=2, num_actions=board_size * board_size + 1
        )

        try:
            checkpoint = torch.load(model_path, map_location='cpu')
        except FileNotFoundError:
            print(f"❌ 模型文件不存在: {model_path}（将使用占位逻辑）")
            return None
        except Exception as e:
            print(f"❌ 模型文件读取失败 ({model_path}): {e}（将使用占位逻辑）")
            return None
        ...
        print(f"❌ AI 模型加载失败 ({model_path}): {e}（将使用占位逻辑）")
        return None


def _ai_select_move(model, engine, color, temperature=1.0):
    """
    让单个 AI 模型选择一手棋。

    Args:
        model: 加载好的策略网络；None 时使用占位逻辑（靠近天元）
        engine: GoEngine 实例
        color: 当前行棋方（1=黑, 2=白）
        temperature: 采样温度；<=0 表示贪心（取概率最高的合法动作）

    Returns:
        (row, col) 表示落子；None 表示 Pass。

    合法性说明：GoEngine.get_legal_moves 已经过滤了占用、自杀和打劫违规，
    因此这里只从合法落子点中选择；Pass（索引 size*size）也参与比较，
    若模型认为 Pass 概率最高则停手；所有落子点都非法时自动 Pass。
    """
    import numpy as np

    legal = engine.get_legal_moves(color)
    pass_index = engine.size * engine.size  # 19 路棋盘 = 361

    # 占位逻辑：没有可用模型时选最靠近天元的合法点
    if model is None:
        if not legal:
            return None
        center = (engine.size // 2, engine.size // 2)
        return min(legal, key=lambda pos: (pos[0] - center[0]) ** 2 + (pos[1] - center[1]) ** 2)

    # 候选动作 = 全部合法落子 + Pass（None）
    candidates = list(legal) + [None]
    if len(candidates) == 1:
        return None  # 只有 Pass 可选

    try:
        # 1. 构建模型输入并推理
        state = torch.tensor(
            board_to_model_input(engine.board, color),
            dtype=torch.float32
        ).unsqueeze(0)
        ...
        for move in candidates:
            if move is None:
                ...
                row, col = move
                idx = row * engine.size + col
            scores.append(float(probs[idx]))

        # 3. 选择动作
        if temperature is not None and temperature > 0:
            # 温度采样：probs^(1/T) 再按候选子集归一化，
            # 等价于对 logits/T 做 softmax 后只看合法动作的概率。
            scaled = [s ** (1.0 / temperature) for s in scores]
            total = sum(scaled)
            if total > 0:
                weights = [s / total for s in scaled]
                chosen = int(np.random.choice(len(candidates), p=weights))
            else:
                chosen = int(np.argmax(scores))  # 数值异常时退化为贪心
        else:
            # 贪心：选概率最高的合法动作（含 Pass）
            chosen = int(np.argmax(scores))

        return candidates[chosen]
    except Exception as e:
        print(f"AI 推理失败: {e}（退回占位逻辑）")
        if not legal:
            return None
        center = (engine.size // 2, engine.size // 2)
        return min(legal, key=lambda pos: (pos[0] - center[0]) ** 2 + (pos[1] - center[1]) ** 2)
...
def ai_vs_ai():
    """
    功能 6：两个 AI 各用一个模型文件对弈（AI 1 执黑，AI 2 执白）。

    使用与现有模式一致的 Pygame 界面渲染棋盘（复用 GoPygame 的绘制逻辑），
    AI 推理、合法点过滤、终局判定与计分逻辑保持不变。
    """
    # ==================== 配置（可按需修改） ==================6==
    AI_1_MODEL_PATH = "./checkpoints/best_model.pth"  # 黑方模型（默认与白方相同，未来可改为不同文件）
    AI_2_MODEL_PATH = "./checkpoints/best_model (1).pth"  # 白方模型（未来可加载第二个不同的模型）
    TEMPERATURE = 1.0   # 采样温度：1.0=按概率随机采样；0.0 或负数=贪心
    MOVE_DELAY = 0.5    # 每步之间的延迟（秒），让对局可观看
    MAX_MOVES = 500     # 最大手数上限，防止无限对局
    BOARD_SIZE = 19     # 棋盘大小（模型按 19 路训练）
    # ============================================================

    print("\n" + "=" * 60)
    print("  AI vs AI（双模型）")
    print(f"  黑方模型 (AI 1): {AI_1_MODEL_PATH}")
    print(f"  白方模型 (AI 2): {AI_2_MODEL_PATH}")
    print(f"  温度: {TEMPERATURE} | 每步延迟: {MOVE_DELAY}s | 最大手数: {MAX_MOVES}")
    print("=" * 60)

    # 图形界面依赖检查（与 GoPygame 一致）
    if pygame is None:
        print("❌ 未安装 pygame，无法启动 AI vs AI 图形界面（pip install pygame）")
        return

    # 1. 加载两个模型（各自独立路径；失败则降级为占位逻辑）
    model_black = _load_ai_model(AI_1_MODEL_PATH, BOARD_SIZE)
    model_white = _load_ai_model(AI_2_MODEL_PATH, BOARD_SIZE)
    if model_black is None:
        print("⚠️ 黑方模型不可用，黑方将使用占位逻辑")
    if model_white is None:
        print("⚠️ 白方模型不可用，白方将使用占位逻辑")

    # 2. 创建 Pygame 窗口并复用现有棋盘渲染
    #    以 ai_black=False / ai_white=False 创建，避免重复加载模型；
    #    随后把本模式的 GoEngine 挂到显示对象上，规则与计分逻辑完全由本模式控制。
    display = GoPygame(BOARD_SIZE, ai_black=False, ai_white=False)
    engine = GoEngine(BOARD_SIZE)
    display.engine = engine
    display.ai_black = True
    display.ai_white = True
    display.ai_vs_ai_mode = True
    pygame.display.set_caption(f"围棋 AI vs AI（双模型）- {BOARD_SIZE}路")

    # 3. 对局循环
    move_count = 0
    display.render()  # 先画出空棋盘
    ...
    while not engine.game_over and move_count < MAX_MOVES:
        ...
        model = model_black if color == 1 else model_white
        ai_label = "AI 1 (黑)" if color == 1 else "AI 2 (白)"

        move = _ai_select_move(model, engine, color, TEMPERATURE)

        if move is None:
            # Pass：可能因为模型选 361，或所有落子点都非法
            result = engine.pass_move(color)
            info = f"{ai_label} 停手 (Pass)"
        else:
            row, col = move
            result = engine.place_stone(row, col, color)
            if not result['success']:
                # 防御性处理：正常不会发生（已过滤合法点）
                print(f"{ai_label} 动作 ({row}, {col}) 非法（{result['message']}），自动停手")
                result = engine.pass_move(color)
                info = f"{ai_label} 动作非法，自动停手"
            else:
                coord = f"{chr(ord('A') + col)}{BOARD_SIZE - row}"
                info = f"{ai_label} → {coord}"

        move_count += 1
        # 控制台日志 + 窗口信息（窗口标题与底部消息均显示当前动作）
        print(f"{info} | 手数: {move_count} | 提子: 黑 {engine.captures[1]} 白 {engine.captures[2]}")
        display.show_message(info)
        pygame.display.set_caption(
            f"{info} | 手数: {move_count} | 提子: 黑 {engine.captures[1]} 白 {engine.captures[2]}"
        )
        display.render()

        if engine.game_over:
            break
        pygame.time.wait(int(MOVE_DELAY * 1000))  # 延迟但不阻塞事件处理

    # 4. 终局与计分
    if not engine.game_over:
        engine.game_over = True
        print(f"⏹️ 达到最大手数 {MAX_MOVES}，对局强制结束")

    # 简单计分：不人工标记死子，默认棋盘上所有棋子视为活子
    scores = engine.get_score()
    territory = engine.get_territory()
    winner = engine.get_winner()

    print("\n" + "=" * 60)
    print("  对局结束")
    print(f"  总手数: {len(engine.history)}")
    print(f"  领地: 黑 {territory['black']}  白 {territory['white']}"
          f"（公空 {territory['dame']}）")
    print(f"  总分(中国规则, 白贴 {scores['komi']:g}): "
          f"黑 {scores['black']:g}  白 {scores['white']:g}")
    if winner == 1:
        diff = scores['black'] - scores['white']
        result_text = f"🏆 AI 1（黑）获胜（{diff:g} 子）"
    elif winner == 2:
        diff = scores['white'] - scores['black']
        result_text = f"🏆 AI 2（白）获胜（{diff:g} 子）"
    else:
        result_text = "🤝 平局"
    print(f"  {result_text}")
    print("=" * 60)

    # 5. 在窗口上显示结果，等待玩家按 ESC 或关闭窗口退出
    display.show_message(result_text)
    pygame.display.set_caption(f"对局结束 | {result_text}")
    display.render()
...
# ============================================================
#  功能 7：纯策略网络 vs MCTS（对战模式）
# ============================================================

def _load_value_model(model_path, board_size=19):
    """
    加载价值网络（ValueNetworkAlphaGoLee），失败时打印错误并返回 None。
    ...
    try:
        ...
        try:
            ...
            print(f"❌ 价值网络模型文件不存在: {model_path}")
            return None
        except Exception as e:
            print(f"❌ 价值网络模型文件读取失败 ({model_path}): {e}")
            return None
        ...
        model.eval()
        print(f"✅ 价值网络已加载: {model_path}")
        return model
    except Exception as e:
        print(f"❌ 价值网络加载失败 ({model_path}): {e}")
        return None


def select_move_policy_only(model, board, current_player, device, history=None):
    """
    纯策略网络选点（贪心，不使用 MCTS/价值网络）。

    流程：策略网络输出 [1, 362] logits -> softmax -> 屏蔽非法动作 -> argmax。
    非法动作（占用/自杀/打劫）由 MCTS.get_legal_moves 过滤，Pass 始终合法。
    传入 history（对局级局面指纹集合）可启用与 GoEngine 一致的位置超劫判定，
    防止 AI 走出会复现整局更早局面的“非法”打劫落子。

    Returns:
        move: 0~360（棋盘交点）或 361（Pass）
    """
    import numpy as np
    from src.mcts import board_to_state, get_legal_moves, PASS_MOVE

    size = len(board)
    legal_moves = get_legal_moves(board, current_player, history)  # 0~360 + 361(Pass)

    # 模型不可用时降级为占位逻辑：选最靠近天元的合法落子点
    if model is None:
        candidates = [m for m in legal_moves if m != PASS_MOVE]
        if not candidates:
            return PASS_MOVE
        center = (size // 2, size // 2)
        return min(
            candidates,
            key=lambda m: (m // size - center[0]) ** 2 + (m % size - center[1]) ** 2,
        )
    ...
    with torch.no_grad():
        logits = model(state_tensor)  # [1, 362]
        probs = torch.softmax(logits, dim=1).squeeze(0).cpu().numpy()
...
def select_move_mcts(mcts, board, current_player, game_history=None):
    """MCTS 选点（贪心）：直接返回搜索后访问次数最多的动作。"""
    return mcts.select_move(board, current_player, temperature=0.0, game_history=game_history)


def policy_vs_mcts():
    """
    功能 7：纯策略网络（贪心） vs MCTS（策略网络 + 价值网络 + 蒙特卡洛树搜索）。

    黑方 = 纯策略网络（快速贪心），白方 = MCTS（搜索后落子）。
    用 Pygame 渲染棋盘（复用 GoPygame 绘制逻辑），展示 MCTS 搜索带来的棋力提升。
    """
    # ==================== 配置（可按需修改） ====================
    POLICY_MODEL_PATH = "./checkpoints/best_model.pth"       # 策略网络（双方共用）
    VALUE_MODEL_PATH = "./checkpoints_value/best_model .pth"  # 价值网络（MCTS 使用）
    MCTS_SIMULATIONS = 400    # CPU: 100，GPU: 400
    DEVICE = "cpu"            # 可改为 "cuda"（有 GPU 时；不可用时 MCTS 会自动回退到 CPU）
    MOVE_DELAY = 1.07          # 每步之间的延迟（秒），让对局可观看
    MAX_MOVES = 500           # 最大手数上限，防止无限对局
    BOARD_SIZE = 19           # 棋盘大小（模型按 19 路训练）
    # ============================================================
    ...
    print("\n" + "=" * 60)
    print("  纯策略网络 vs MCTS")
    print(f"  黑方（纯策略网络）: {POLICY_MODEL_PATH}")
    print(f"  白方（MCTS）: 策略 {POLICY_MODEL_PATH} + 价值 {VALUE_MODEL_PATH}")
    print(f"  MCTS 模拟次数: {MCTS_SIMULATIONS} | 设备: {DEVICE}")
    print("=" * 60)

    # 图形界面依赖检查（与 GoPygame 一致）
    if pygame is None:
        print("❌ 未安装 pygame，无法启动图形界面（pip install pygame）")
        return

    # 1. 导入 MCTS 工具（MCTS.py 依赖 torch/numpy）
    try:
        ...
        # board_to_state / get_legal_moves 在 select_move_policy_only 内部导入，
        # 避免本文件在未安装 torch 时无法导入（保持与现有惰性导入风格一致）
    except ImportError as e:
        print(f"❌ 无法导入 MCTS.py: {e}（请确认 MCTS.py 与本文件在同一目录）")
        return

    # 2. 加载模型（MCTS 需要策略 + 价值两个模型，失败则提示并退出）
    policy_model = _load_ai_model(POLICY_MODEL_PATH, BOARD_SIZE)
    value_model = _load_value_model(VALUE_MODEL_PATH, BOARD_SIZE)
    if policy_model is None or value_model is None:
        print("❌ 模型加载失败，无法开始对局（请检查模型路径）")
        return
    policy_model.to(DEVICE)

    # 3. 创建 MCTS（构造时会把两个模型移动到 DEVICE 并设为 eval 模式）
    mcts = MCTSSearch(
    ...
    display.ai_white = True
    display.ai_vs_ai_mode = True
    display.mode_title = "🤖 纯策略网络 vs MCTS"
    pygame.display.set_caption(f"纯策略网络(黑) vs MCTS(白) - {BOARD_SIZE}路")

    # 5. 对局循环
    move_count = 0
    # 对局级劫历史（与 GoEngine 的位置超劫判定一致）：从空盘开始，
    # 每手棋（含提子）产生的新局面都加入；Pass 不改变棋盘，不加入新指纹
    game_history = {get_position_fingerprint(engine.board)}
    display.render()  # 先画出空棋盘
    ...
    while not engine.game_over and move_count < MAX_MOVES:
        ...
        color = engine.current_player
        ai_label = "纯策略网络 (黑)" if color == 1 else "MCTS (白)"

        # 先显示“思考中”，再开始推理（MCTS 需要数秒）
        pygame.display.set_caption(f"{ai_label} 思考中... | 手数: {move_count + 1}")
        display.show_message(f"{ai_label} 思考中...")
        display.render()

        # 各自的动作选择逻辑：黑 = 纯策略贪心，白 = MCTS 搜索
        if color == 1:
            move = select_move_policy_only(
                policy_model, engine.board, color, DEVICE,
                history=game_history,
            )
        else:
            move = select_move_mcts(mcts, engine.board, color, game_history=game_history)
        ...
        # 每手棋（含提子）产生的新局面加入对局历史，供下一次合法点过滤使用
        if move != PASS_MOVE and result.get('success', False):
            game_history.add(get_position_fingerprint(engine.board))
    ...
    # 6. 终局与计分（胜负用 MCTS.score_game 的中国规则数子法）
    if not engine.game_over:
        engine.game_over = True
        print(f"⏹️ 达到最大手数 {MAX_MOVES}，对局强制结束")
    ...
    territory = engine.get_territory()
    scores = engine.get_score()
    winner = score_game(engine.board)  # 1=黑胜, 2=白胜, 0=平局
    ...
    print(f"  总分(中国规则, 白贴 {scores['komi']:g}): "
          f"黑 {scores['black']:g}  白 {scores['white']:g}")
    if winner == 1:
        result_text = "🏆 纯策略网络（黑）获胜！"
    elif winner == 2:
        result_text = "🏆 MCTS（白）获胜！"
    else:
        result_text = "🤝 平局"
    print(f"  {result_text}")
    print("=" * 60)

    # 7. 在窗口上显示结果，等待玩家按 ESC 或关闭窗口退出
    display.show_message(result_text)
    pygame.display.set_caption(f"对局结束 | {result_text}")
    display.render()
...
def main():
    ...
    print("  5. AI vs AI (快速)")
    print("  6. AI vs AI (双模型，黑白各用一个模型文件)")
    print("  7. 纯策略网络 vs MCTS (展示 MCTS 搜索优势)")

    choice = input("请输入 (1-7): ").strip()
    # 模型路径（根据你的实际路径修改）
    model_path = "./checkpoints/best_model.pth"  # 或者用 best_model.pth
    ...
    if choice == '1':
        ...
        # 功能 6：双模型 AI 对弈（纯控制台模式，AI 1 执黑 / AI 2 执白）
        ai_vs_ai()
    elif choice == '7':
        # 功能 7：纯策略网络（黑）vs MCTS（白）
        policy_vs_mcts()
    else:
